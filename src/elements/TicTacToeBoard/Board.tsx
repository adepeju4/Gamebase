...
function Board({ channel, rivalName }: BoardProps) {
  ...
  const applyOpponentMove = (move: MoveEvent) => {
    ...
    if (type === 'move') {
      if (data) {
        const { winner, player, turn, rows, id, isBoardFull } = data;
        ...
        if (gameState.rows[data.rowIndex][data.columnIndex] === '' && player !== gameState.player) {
          setGameState(prev => {
            return {
              ...prev,
  ...
  const handleGameReset = () => {
    const nextStarter =
      gameOutcome === 'loss' ? gameState.player : gameState.player === 'X' ? 'O' : 'X';
    resetGame(nextStarter);
    setGameOutcome('');
    setrivalMessage('');
  ...
  return (
    <div className="flex-[.7] p-8 flex flex-col justify-center items-center h-full">
      <div>
        ...
        <h3 className="text-center text-white text-large">{client?.user?.name} (Me)</h3>
      </div>
